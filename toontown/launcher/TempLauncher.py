...
class TempLauncher(LauncherBase):
    notify = DirectNotifyGlobal.directNotify.newCategory('TempLauncher:')
    ...
    def __init__(self):
        ...
        if not os.path.exists('userdata/logs/'):
            os.mkdir('userdata/logs/')
            self.notify.info('Made new directory to save logs.')

        logfile = os.path.join('userdata/logs', self.logPrefix + logSuffix + '.log')
    ...
    def getPlayToken(self):
        return self.getValue('PLAYTOKEN')

    def getGameServer(self):
        return self.getValue('PLAYSERVER')
